...
public:
  PathPlannerNode() : Node("path_planner_node")
  {
    ...
    const auto obs_x = this->declare_parameter<std::vector<double>>("obstacle_x", {});
    const auto obs_y = this->declare_parameter<std::vector<double>>("obstacle_y", {});
    const auto obs_r = this->declare_parameter<std::vector<double>>("obstacle_r", {});
