...
// ============================================================
#include "uav_navigation/ekf.hpp"
...
EKF::EKF() {
    ...
    Q_.block<3, 3>(9, 9) *= 1e-5;    // Accel Bias Random Walk,센서 바이어스 오차는 시간이 흐르며 조금씩 변하므로, 1e-5->1e-3
    // 아주 작은 값을 더해줌으로써 EKF가 바이어스 추정치를 조금씩 변화시킬 수 있도록 허용 (너무 작으면 바이어스가 고정되어 버릴 수 있음)
    // 가속도 센서의 고유 오차(Bias)는 아주 천천히 변하니, 너무 예민하게 EKF가 반응하는것을 금지
    // 예를 들어 Q 값을 계속 크게 놔두면, 실제 급감/가속 할 때 오차로 잘못 판단할 수 있으므로 추정 알고리즘이 망가져 버림
    // 따라서 이 값을 낮춰서 바이어스 추정을 무겁고 보수적으로 둠 
